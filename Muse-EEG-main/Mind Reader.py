...
def initiate_tf():
    ...
    path = "Models/"
    lite_file = "ei-lslsls0001-project-1-nn-classifier-tensorflow-lite-float32-model.lite"
...
def inference():
    ...
    print(left)
    print(right)
    # checking if over confidence thresholds
    if left >= confidence_threshold and not math.isnan(left):
        choice = "Left"
        print("Left")
        state = -1
    elif right >= confidence_threshold and not math.isnan(right):
        choice = "Right"
        print("Right")
        state = 1
    else:
